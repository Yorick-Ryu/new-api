...
// API/business addresses and origins can change an implicit RP ID.
func IsPasskeyDomainOption(key string) bool {
	return key == "passkey.rp_id" || key == "passkey.legacy_rp_ids" || key == "passkey.origins" || key == "ServerAddress" || key == "SiteAddress"
}
...
func lockPasskeyDomainSettings(tx *gorm.DB) (system_setting.PasskeySettings, string, string, error) {
	common.OptionMapRWMutex.RLock()
	settings, serverAddress, siteAddress := *system_setting.GetPasskeySettings(), system_setting.ServerAddress, system_setting.SiteAddress
	...
	if err := tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&rows).Error; err != nil {
		return settings, serverAddress, siteAddress, err
	}
	for i := range rows {
	...
	serverAddress, siteAddress = rows[0].Value, rows[1].Value
	settings.LegacyRPIDs, settings.Origins, settings.RPID = rows[2].Value, rows[3].Value, rows[4].Value
	return settings, serverAddress, siteAddress, nil
}

func validatePasskeyRPIDWithTx(tx *gorm.DB, rpID string) error {
	settings, serverAddress, siteAddress, err := lockPasskeyDomainSettings(tx)
	if err != nil {
		return err
	}
	if siteAddress != "" {
		serverAddress = siteAddress
	}
...
func UpdatePasskeyDomainOptions(values map[string]string, preview bool, confirmation string) (*PasskeyDomainChange, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		businessAddress := siteAddress
		if businessAddress == "" {
			businessAddress = serverAddress
		}
		previous := settings.WithDefaults(businessAddress)
		previousBusinessAddress := businessAddress
		before := [5]string{serverAddress, siteAddress, settings.RPID, settings.LegacyRPIDs, settings.Origins}
		if value, changed := values["ServerAddress"]; changed {
			serverAddress = strings.TrimRight(strings.TrimSpace(value), "/")
		}
		if value, changed := values["SiteAddress"]; changed {
			siteAddress = strings.TrimRight(strings.TrimSpace(value), "/")
		}
		businessAddress = siteAddress
		if businessAddress == "" {
			businessAddress = serverAddress
		...
		if value, changed := values["passkey.origins"]; changed {
		...
		} else if previousBusinessAddress != businessAddress && (settings.Origins == "" || settings.Origins == "[]") {
			...
			origins := []string{}
			for _, origin := range []string{previous.Origins, settings.WithDefaults(businessAddress).Origins} {
				if origin != "" && !slices.Contains(origins, origin) {
					origins = append(origins, origin)
				}
			}
		...
		if value, changed := values["passkey.rp_id"]; changed {
			...
			if settings.RPID != "" {
				settings.RPID, err = system_setting.NormalizePasskeyRPID(settings.RPID, settings.WithDefaults(businessAddress).Origins)
				if err != nil {
					return err
				}
			}
		...
		effective := settings.WithDefaults(businessAddress)
		...
		nextIDs := settings.WithDefaults(businessAddress).RelyingPartyIDs()
		...
		payload, err := common.Marshal(struct {
			...
			Removed        []string
			Known, Unknown int64
		}{before, [5]string{serverAddress, siteAddress, settings.RPID, settings.LegacyRPIDs, settings.Origins}, change.RemovedRPIDs, change.AffectedCredentials, change.UnknownCredentials})
		if err != nil {
			return err
		}
		...
		values["ServerAddress"], values["SiteAddress"] = serverAddress, siteAddress
...
func applyPasskeyDomainOptions(values map[string]string) {
	...
	for _, key := range []string{"ServerAddress", "SiteAddress", "passkey.legacy_rp_ids", "passkey.origins", "passkey.rp_id"} {
