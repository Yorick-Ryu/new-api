...
func TestModelDetailsVisibilitySetting(t *testing.T) {
	const key = "LogResponseModelDisplayMode"
	original := common.LogResponseModelDisplayMode.Load()
	require.Equal(t, common.LogResponseModelDisplayOff, original, "response models are hidden by default")
	...
	t.Cleanup(func() {
		common.LogResponseModelDisplayMode.Store(original)
		common.OptionMapRWMutex.Lock()
	...
	hidden := `{"request_path":"/v1/responses","large_integer":9007199254740993}`
	for _, mode := range []string{"off", "on", "admin_only", "off"} {
		t.Run(mode, func(t *testing.T) {
			require.NoError(t, validateOptionValue(key, mode))
			require.NoError(t, updateOptionMap(key, mode))
			...
			assert.JSONEq(t, expectedUser, users[0].Other)
			assert.Equal(t, "codex-auto-review", users[0].ModelName)
			expectedAdmin := stored
			if mode == "off" {
				expectedAdmin = hidden
			}
			admins := []*Log{{Other: stored}}
			FormatAdminLogs(admins)
			assert.JSONEq(t, expectedAdmin, admins[0].Other)
			roots := []*Log{{Other: stored}}
			FormatRootLogs(roots)
			assert.JSONEq(t, expectedAdmin, roots[0].Other)
		})
	}
	for _, invalid := range []string{"", "true", "false", "everyone"} {
		assert.Error(t, UpdateOption(key, invalid))
		assert.Error(t, updateOptionMap(key, invalid))
		assert.Equal(t, "off", common.LogResponseModelDisplayMode.Load())
		assert.Equal(t, "off", common.OptionMap[key])
	}
	assert.Error(t, UpdateOption("LogModelDetailsAdminOnlyEnabled", "true"))
}
