...
func setupPasskeyDomainOptions(t *testing.T) {
	...
	previousSiteAddress := system_setting.SiteAddress
	common.OptionMapRWMutex.RUnlock()
	t.Cleanup(func() {
		common.OptionMapRWMutex.Lock()
		common.OptionMap = previousOptions
		system_setting.ServerAddress = previousAddress
		system_setting.SiteAddress = previousSiteAddress
...
func TestPasskeyDomainUpdatesPreserveSeparatedAPIAndBusinessOrigins(t *testing.T) {
	for _, explicit := range []bool{false, true} {
		t.Run(fmt.Sprintf("explicit=%t", explicit), func(t *testing.T) {
			...
			system_setting.SiteAddress = "https://example.com"
			if explicit {
				settings.RPID, settings.Origins = "example.com", "https://example.com,https://www.example.com"
			}
			require.NoError(t, model.UpdateOption("ServerAddress", "https://api.other.example"))
			snapshot := system_setting.PasskeySettingsSnapshot()
			assert.Equal(t, "example.com", snapshot.EffectiveRPID())
			assert.NotContains(t, snapshot.Origins, "api.")
			assert.Empty(t, snapshot.LegacyRPIDs)
			require.NoError(t, model.UpdateOption("SiteAddress", "https://new.example.com"))
			snapshot = system_setting.PasskeySettingsSnapshot()
			if explicit {
				assert.Equal(t, "example.com", snapshot.EffectiveRPID())
				assert.Equal(t, "https://example.com,https://www.example.com", snapshot.Origins)
			} else {
				assert.Equal(t, []string{"new.example.com", "example.com"}, snapshot.RelyingPartyIDs())
				assert.Equal(t, "https://example.com,https://new.example.com", snapshot.Origins)
			}
			assert.Equal(t, "https://api.other.example", system_setting.ServerAddress)
			assert.Equal(t, "https://new.example.com", system_setting.SiteAddress)
		})
	}
}
