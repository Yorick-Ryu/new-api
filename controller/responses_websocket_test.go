...
func TestResponsesStreamOutcomesPreserveAccounting(t *testing.T) {
	for _, transport := range []string{"websocket", "http-sse"} {
		for _, tc := range []struct {
			...
			{name: "failed-output-is-not-estimated", expression: `tier("output", c * 2000)`, terminal: `{"type":"response.failed","response":{"id":"first","status":"failed","usage":null}}`, delta: true, failed: true},
			...
			t.Run(transport+"/"+tc.name, func(t *testing.T) {
				...
				fixture := newResponsesWSBillingTest(t, tc.expression, func(ws *websocket.Conn, _ *http.Request) {
					for turn := 0; ; turn++ {
						...
						assert.NotContains(t, body, "stream_id", "client correlation metadata must stay local")
				...
				quotas := []int{1000}
				if tc.name == "failed-output-is-not-estimated" {
					quotas[0] = 0
				}
				if tc.failed {
