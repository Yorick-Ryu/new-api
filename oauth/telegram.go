...
func NewTelegramOAuthFlow(contexts ...*gin.Context) (*TelegramOAuthFlow, error) {
	if err := TelegramConfigurationError(); err != nil {
		return nil, err
	}
	redirectURI := strings.TrimRight(system_setting.GetSiteAddress(), "/") + "/oauth/telegram"
	if len(contexts) > 0 && contexts[0] != nil {
		var err error
		redirectURI, err = callbackURL(contexts[0], "telegram")
		if err != nil {
...
func (p *TelegramProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	redirectURI, err := callbackURL(c, "telegram")
	if err != nil {
