...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	if relayInfo.BillingSource == BillingSourceSubscription && relayInfo.SubscriptionGroupRatio > 0 {
		actualGroupRatio = relayInfo.SubscriptionGroupRatio
	}
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	if usage.InputTokens > 0 {
		relayInfo.PerformanceInputTokens = int64(usage.InputTokens)
	}
	relayInfo.PerformanceCacheReadTokens = int64(usage.PromptTokensDetails.CachedTokens)
}
...
func postConsumeQuotaWithResult(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (result postConsumeQuotaResult, err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		delta := int64(quota)
		if relayInfo.SubscriptionModelMultiplier > 0 {
			before, err := model.SubscriptionQuotaWithMultiplier(int64(preConsumedQuota), relayInfo.SubscriptionModelMultiplier)
			if err != nil {
				return result, err
			}
			after, err := model.SubscriptionQuotaWithMultiplier(int64(preConsumedQuota)+int64(quota), relayInfo.SubscriptionModelMultiplier)
			if err != nil {
				return result, err
			}
			delta = after - before
