...
func TestResponsesUsageAccumulatorInterruptedTextFallback(t *testing.T) {
	for _, withUsage := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(t, !withUsage, usage.CompletionTokens == 1)
		})
	}
}
...
func TestObserveResponsesOutcomeRecordsProtocolFacts(t *testing.T) {
	for _, tc := range []struct {
		...
		{"nested sse error", `{"type":"error","error":{"code":"context_length_exceeded","type":"invalid_request_error","message":"too long"}}`, relaycommon.ResponseOutcomeFailed, "context_length_exceeded", "invalid_request_error", ""},
