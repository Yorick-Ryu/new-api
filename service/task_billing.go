...
func appendTaskSubscriptionCharge(other *model.LogOther, task *model.Task, beforeQuota, afterQuota int) {
	if !taskIsSubscription(task) {
		return
	}
...
func taskBillingOther(task *model.Task) *model.LogOther {
	...
	if taskIsSubscription(task) {
		...
		if task.PrivateData.SubscriptionGroupRatio > 0 {
			other.SetPublic("subscription_group_ratio", task.PrivateData.SubscriptionGroupRatio)
			if task.PrivateData.SubscriptionOriginalGroupRatio != nil {
				other.SetPublic("subscription_original_group_ratio", *task.PrivateData.SubscriptionOriginalGroupRatio)
			}
		} else {
			other.SetPublic("subscription_model_multiplier", multiplier)
		}
	}
