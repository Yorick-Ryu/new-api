...
// ResponsesTransportFromContext preserves the transport selected by a WS
// worker even though its request runs through the shared POST middleware.
func ResponsesTransportFromContext(c *gin.Context) constant.ResponsesTransport {
	if c == nil {
		return constant.ResponsesTransportNone
	}
	if constraints, ok := common.GetContextKeyType[*dto.ChannelConstraints](c, constant.ContextKeyChannelConstraints); ok && constraints != nil {
		for _, filter := range constraints.Filters {
			if filter.Kind == dto.FilterResponsesTransport && filter.ResponsesTransport != constant.ResponsesTransportNone {
				return filter.ResponsesTransport
			}
		}
	}
	return ResponsesTransportFromRequest(c.Request)
}
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	constraints := GetChannelConstraints(param.Ctx)
	transport := param.ResponsesTransport
	if transport == constant.ResponsesTransportNone && param.Ctx != nil {
		transport = ResponsesTransportFromContext(param.Ctx)
	}
	if transport != constant.ResponsesTransportNone {
		constraints.AddFilter(dto.ChannelFilter{Kind: dto.FilterResponsesTransport, ResponsesTransport: transport})
	}
...
func SelectChannelForRequest(c *gin.Context, modelName string, retry *RetryParam) (*model.Channel, string, *ChannelSelectError) {
	...
	transport := retry.ResponsesTransport
	if transport == constant.ResponsesTransportNone && c != nil {
		transport = ResponsesTransportFromContext(c)
	}
	if transport != constant.ResponsesTransportNone {
		constraints.AddFilter(dto.ChannelFilter{Kind: dto.FilterResponsesTransport, ResponsesTransport: transport})
	}
