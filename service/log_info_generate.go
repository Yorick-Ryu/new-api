...
func appendBillingInfo(relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if relayInfo.BillingSource == "subscription" {
		if relayInfo.SubscriptionGroupRatio > 0 {
			other.SetPublic("subscription_group_ratio", relayInfo.SubscriptionGroupRatio)
			if relayInfo.SubscriptionOriginalGroupRatio != nil {
				other.SetPublic("subscription_original_group_ratio", *relayInfo.SubscriptionOriginalGroupRatio)
			}
		} else if relayInfo.SubscriptionModelMultiplier > 0 {
			other.SetPublic("subscription_model_multiplier", relayInfo.SubscriptionModelMultiplier)
		}
