/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as published by
the Free Software Foundation, either version 3 of the License, or (at your
option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { cleanup, render, screen } from '@testing-library/react'
import { createInstance } from 'i18next'
import { I18nextProvider } from 'react-i18next'
import { afterEach, expect, it } from 'vitest'

import { SubscriptionQuotaUsage } from '../subscription-quota-usage'

const i18n = createInstance()
await i18n.init({ lng: 'en', resources: { en: { translation: {} } } })
afterEach(cleanup)

it.each([
  { used: 10, remaining: 90 },
  { used: 0, remaining: 100 },
  { used: 120, remaining: 0 },
])(
  'shows $remaining% remaining when $used of 100 units have been used',
  ({ used, remaining }) => {
    render(
      <I18nextProvider i18n={i18n}>
        <SubscriptionQuotaUsage
          label='Weekly quota'
          amountUsed={used}
          amountTotal={100}
          nextResetTime={2100000000}
          isActive
          variant='remaining'
        />
      </I18nextProvider>
    )
    expect(screen.getByText(`${remaining}%`)).toBeTruthy()
    expect(screen.getByText('Remaining')).toBeTruthy()
    const progress = screen.getByRole('progressbar', { name: 'Weekly quota' })
    expect(progress.getAttribute('aria-valuenow')).toBe(String(remaining))
    expect(progress.getAttribute('aria-valuetext')).toBe(
      `${remaining}% Remaining`
    )
    expect(
      progress
        .querySelector('[data-slot="progress-indicator"]')
        ?.classList.contains('bg-primary')
    ).toBe(true)
    const reset = screen.getByText(/Next reset:/)
    expect(reset.textContent).toMatch(/\d{1,2}:\d{2}/)
    expect(reset.textContent).not.toMatch(/\d{1,2}:\d{2}:\d{2}/)
    expect(
      progress.compareDocumentPosition(reset) & Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy()
  }
)

it.each([
  { active: true, total: 0 },
  { active: false, total: 100 },
])(
  'hides remaining percentage and progress when active=$active and limit=$total',
  ({ active, total }) => {
    render(
      <I18nextProvider i18n={i18n}>
        <SubscriptionQuotaUsage
          label='Weekly quota'
          amountUsed={10}
          amountTotal={total}
          nextResetTime={2100000000}
          isActive={active}
          variant='remaining'
        />
      </I18nextProvider>
    )
    expect(screen.queryByRole('progressbar')).toBeNull()
    expect(screen.queryByText('Remaining')).toBeNull()
    if (total === 0) expect(screen.getByText('Unlimited')).toBeTruthy()
    if (!active) expect(screen.queryByText(/Next reset:/)).toBeNull()
    expect(screen.getByText(/^Used /)).toBeTruthy()
  }
)

it('keeps the existing usage presentation when no remaining variant is selected', () => {
  render(
    <I18nextProvider i18n={i18n}>
      <SubscriptionQuotaUsage
        label='Weekly quota'
        amountUsed={10}
        amountTotal={100}
        isActive
      />
    </I18nextProvider>
  )
  expect(
    screen
      .getByRole('progressbar', { name: 'Weekly quota' })
      .getAttribute('aria-valuenow')
  ).toBe('10')
  expect(screen.queryByText('Remaining')).toBeNull()
})
