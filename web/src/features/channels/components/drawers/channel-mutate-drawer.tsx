  ...
  const selectProvider = useCallback(
    (target: ChannelProviderTarget) => {
      ...
      form.setValue(
        ...
        normalizeResponsesTransport(
          nextType,
          form.getValues('responses_transport')
        ),
        { shouldDirty: true }
      )
  ...
  const routingFields = (
    ...
    >
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{t('Responses API transport')}</FormLabel>
            <Select
            ...
            >
              ...
              <SelectContent>
                <SelectItem
                  value='both'
                  disabled={!supportsResponsesWebSocket(currentType)}
                >
                  {t('HTTP and WebSocket')}
                </SelectItem>
                <SelectItem value='http'>{t('HTTP only')}</SelectItem>
                <SelectItem
                  value='websocket'
                  disabled={!supportsResponsesWebSocket(currentType)}
                >
            ...
            <FormDescription>
              {t('Only matching /v1/responses requests can use this channel')}
