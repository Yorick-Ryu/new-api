...
describe('Responses WebSocket channel setting', () => {
  test.each([undefined, false, true])(
    ...
    (enabled) => {
      ...
      expect(settings.responses_websocket_enabled).toBe(enabled !== false)
    }
  )

  test.each([58, 59, 60, 14])(
    'existing channel type %s requires an explicit WebSocket opt-in',
    (type) => {
      for (const enabled of [undefined, false, true]) {
        ...
        const values = transformChannelToFormDefaults(channel)
        expect(values.responses_websocket_enabled).toBe(
          type !== 14 && enabled === true
        )
        const saved = JSON.parse(buildSettingJSON(values))
        expect(saved.responses_websocket_enabled).toBe(
          type !== 14 && enabled === true
        )
      }
    }
