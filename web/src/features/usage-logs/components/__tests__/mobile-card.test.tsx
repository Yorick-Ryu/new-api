...
it('keeps WebSocket throughput and first-token timing visible without the SSE flag', () => {
  renderLogs({
    logs: [
      {
        ...log,
        is_stream: false,
        other: JSON.stringify({ ws: true, frt: 100 }),
      },
    ],
  })
  expect(screen.getByText('WS')).toBeVisible()
  expect(screen.getByText('533 t/s')).toBeVisible()
