...
beforeEach(() => {
  ...
  window.localStorage.setItem('beiapi-desktop-setup:1', 'confirmed')
...
describe('overview setup guide', () => {
  it('shows usage first and only a header entry when setup is complete', async () => {
    ...
    ).toEqual(['Overview', 'Usage at a glance', 'Subscription Plans'])
  ...
  it('toggles the completed guide with the keyboard and restores focus after hiding it', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole('button', { name: /Download setup tool/ })
      ).toBeVisible()
    )
  ...
  it('keeps the existing progress banner when an incomplete guide is manually collapsed', async () => {
    const user = userEvent.setup()
    window.localStorage.removeItem('beiapi-desktop-setup:1')
    useAuthStore
      .getState()
  ...
  it('removes the collapsed progress banner when desktop setup completes', async () => {
    window.localStorage.removeItem('beiapi-desktop-setup:1')
    useAuthStore.getState().auth.setUser({
      id: 1,
      username: 'dashboard-user',
  ...
  it('does not show a completed setup entry before desktop confirmation', async () => {
    window.localStorage.removeItem('beiapi-desktop-setup:1')
    await renderOverview()
