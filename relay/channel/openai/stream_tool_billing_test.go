...
func TestObserveStreamChoicesDedupesFunctionCallNames(t *testing.T) {
	...
	for _, chunk := range chunks {
		inspectChatStreamChunk(info, chunk, seen, &names)
	}
	...
	inspectChatStreamChunk(info, `{"choices":[{"index":0,"delta":{},"finish_reason":"tool_calls"}]}`, seen, &names)
	...
	inspectChatStreamChunk(filtered, `{"choices":[{"index":0,"delta":{},"finish_reason":"content_filter"}]}`, map[string]struct{}{}, &names)
