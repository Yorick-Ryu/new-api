...
func TestResponseModelSharedResponsesAccumulator(t *testing.T) {
	...
	accumulator.Observe(&dto.ResponsesStreamResponse{Type: "response.created", Response: &dto.OpenAIResponsesResponse{Model: "late"}})
	assert.Equal(t, "returned", info.ResponseModel.ReturnedModel, "settled observations are frozen with usage")
}
